...
public class PluginGen extends MultiWorldChunkGen
{
	...
	public void makeWorld(InternalWorld options) throws InvalidWorldGenOptionsException
	{
		...
		try
		{
			...
			if (generator == null)
			{
				throw new InvalidWorldGenOptionsException("Was not able to find the worldgenenerator with id '" + genId + "' at plugin '" + p.getDescription().getFullName() + "'");
			}
		...
		catch (Throwable e)
		{
			...
			throw (InvalidWorldGenOptionsException) (new InvalidWorldGenOptionsException("Error with custom plugin generator")).initCause(e);
		}
	}
}

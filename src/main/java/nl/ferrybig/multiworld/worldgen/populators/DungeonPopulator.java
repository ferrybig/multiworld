      ...
      for (counterX = dungeonMinXPoint; counterX <= dungeonMaxXPoint; counterX++) {
        for (counterY = dungeonY + dungeonWidthY; counterY >= dungeonMinYPoint; counterY--) {
          for (counterZ = dungeonMinZPoint; counterZ <= dungeonMaxZPoint; counterZ++) {
            ...
            if ((counterX == dungeonMinXPoint) || (counterY == dungeonMinYPoint) || (counterZ
                == dungeonMinZPoint) || (counterX == dungeonMaxXPoint) || (counterY
                == dungeonMaxYPoint) || (counterZ == dungeonMaxZPoint)) {
              // Block is at side
              if ((counterY >= 0) && (!isBlock(
                  world.getBlockAt(counterX, counterY - 1, counterZ).getType()))) {
                // Block is at roof

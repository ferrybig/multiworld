...
public class WorldChatSeperatorPlugin implements Listener, MultiworldAddon {
  ...
  public void onChat(AsyncPlayerChatEvent evt) {
    ...
    InternalWorld w = d.getWorldManager()
        .getInternalWorld(evt.getPlayer().getWorld().getName(), true);
    boolean maySendChat = d.getWorldManager().getFlag(w.getName(), FlagName.SENDCHAT)
        .getAsBoolean();
    if (!maySendChat) {
    ...
    for (InternalWorld world : d.getWorldManager().getLoadedWorlds()) {
      if (world != w && !d.getWorldManager().getFlag(world.getName(), FlagName.RECIEVECHAT)
          .getAsBoolean()) {

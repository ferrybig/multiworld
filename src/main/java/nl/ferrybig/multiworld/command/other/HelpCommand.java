...
public class HelpCommand extends Command {
  ...
  public void runCommand(CommandStack stack) {
    ...
    ChatPaginator.ChatPage page = ChatPaginator
        .paginate(topic.getFullText(stack.getSender()), pageNumber, pageWidth, pageHeight);
    ...
    if (searchString.isEmpty()) {
      for (Map.Entry<String, Command> command : this.data.getPlugin().getCommandHandler().m
        ...
        if (!trimmedTopic.equals("snowman")) {
          possibleMatches.add(command.getValue()
              .generateHelpTopic(stack.getCommandLabel() + " " + trimmedTopic + " .."));
        }
      }
    } else {
      for (Map.Entry<String, Command> command : this.data.getPlugin().getCommandHandler().m
        ...
        if (!trimmedTopic.equals("snowman")) {
          if ((trimmedTopic.length() >= searchString.length()) && (
              Character.toLowerCase(trimmedTopic.charAt(0)) == Character
